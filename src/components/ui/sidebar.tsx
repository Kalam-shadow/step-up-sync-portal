...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "h-7 w-7 flex items-center justify-center transition-all duration-300",
        state === "collapsed" ? "ml-1" : "ml-4", // Adjust margin for alignment
        className
      )}
      onClick={(event) => {
        onClick?.(event);
      ...
      {...props}
    >
      {state === "collapsed" ? <PanelLeft size={18} /> : <PanelLeftClose size={18} />}
  ...
  return (
    <main
      ...
      className={cn(
        ...
        state === "collapsed" ? "ml-[--sidebar-width-icon]" : "ml-[--sidebar-width]",
        className
      )}
      {...props}
    />
