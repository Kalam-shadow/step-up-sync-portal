...
const TrainersPage = () => {
  ...
  const handleEditTrainer = (trainer: Trainer) => {
    const formattedJoiningDate =
      trainer.joiningDate instanceof Date
        ? trainer.joiningDate.toISOString().split("T")[0]
        : trainer.joiningDate;
    ...
    form.setValue("joiningDate", formattedJoiningDate || "");
  ...
  // Add or update trainer mutation
  const mutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: `Failed to ${editingTrainer ? "update" : "add"} trainer: ${
  ...
  if (isError) {
    return <div className="p-8 text-red-500">Error loading trainers!</div>;
  }
